...
export async function POST(request: NextRequest) {
  try {
    ...
    const webhookUrl = process.env.N8N_WEBHOOK_VIDEO_24S_PRODUCT_REAL_LIVE;

    if (!webhookUrl) {
      console.error("N8N_WEBHOOK_VIDEO_24S_PRODUCT_REAL_LIVE is not configured");
      return NextResponse.json(
    ...
    console.log("Creating AI Explains video generation job");
    ...
    if (jobError || !jobData) {
      console.error("Failed to create AI Explains job:", jobError);
    ...
    console.log("Created AI Explains video job:", jobId);
    ...
    console.log("=== AI EXPLAINS VIDEO WEBHOOK REQUEST DEBUG ===");
    ...
    console.log("AI Explains video job created and started");
    return NextResponse.json({
      success: true,
      jobId,
      status: "processing",
      message: "AI Explains Video-Erstellung wurde gestartet",
    });

  } catch (error: any) {
    console.error("Error in ai-explains-video webhook:", error);
    return NextResponse.json(
      {
